...
ninf_40 <- reduce(lapply(t_test, get_n_infectious, 
	lod=40, se=0.99, inf_ct=inf_ct, maxct=maxct, params_df, pop_pars=pop_pars, event_duration=3/24), bind_rows) %>% 
	...
	mutate(mean_baseline_smooth=predict(loess(mean_baseline~t, data=., span=1))) %>%
	mutate(lwr_baseline_smooth=predict(loess(lwr_baseline~t, data=., span=1))) %>%
	mutate(upr_baseline_smooth=predict(loess(upr_baseline~t, data=., span=1))) %>%
	mutate(t=-t)

ninf_35 <- reduce(lapply(t_test, get_n_infectious, 
	lod=35, se=0.95, inf_ct=inf_ct, maxct=maxct, params_df, pop_pars=pop_pars, event_duration=3/24), bind_rows) %>% 
	pivot_wider(names_from="statistic", values_from=c("value")) %>%
	mutate(mean_smooth=predict(loess(mean~t, data=., span=0.6))) %>%
	mutate(lwr_smooth=predict(loess(lwr~t, data=., span=0.6))) %>%
	mutate(upr_smooth=predict(loess(upr~t, data=., span=0.6))) %>%
...
fig_ninf <- ggplot() + 
	...
	geom_line(data=ninf,aes(x=t, y=mean_baseline_smooth), linetype="dashed", alpha=1) +
	# geom_segment(aes(x=0, xend=3, y=pop_pars[["prev"]]*pop_pars[["n_attendees"]],yend=pop_pars[["prev"]]*pop_pars[["n_attendees"]]), linetype="dashed", alpha=1) +
	# coord_cartesian(ylim=c(0,max(ninf$upr)), expand=FALSE) + 
	# coord_cartesian(ylim=c(0,pop_pars[["prev"]]*pop_pars[["n_attendees"]]), expand=FALSE) + 
...
fig_ninf_ic35 <- ggplot() + 
	...
	# geom_ribbon(
	#   data=ninf, 
	#   aes(x=t, ymin=lwr_baseline_smooth, ymax=upr_baseline_smooth, fill=test), alpha=0.2) + 
	geom_point(data=ninf, aes(x=t, y=lwr, col=test), size=0.1, alpha=0) +
	geom_point(data=ninf, aes(x=t, y=upr, col=test), size=0.1, alpha=0) +
	geom_point(data=ninf, aes(x=t, y=mean, col=test), size=0.1, alpha=0) + 
	geom_line(data=ninf, aes(x=t, y=mean, col=test), stat="smooth", method="loess", span=0.6) + 
